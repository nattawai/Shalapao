...
| **ลบรายการ = soft delete เท่านั้น (ไม่มี reversal ใน v0)** · ลบได้เฉพาะงวดที่ยังไม่ปิด · ขาโยกเงินลบทั้งคู่ atomic | `entry.repository.deleteEntry` · reuse `effectiveReconciledLine` (เส้นแม่ทุกชั้น) | กรอกผิดก่อนปิดงวด → ลบทิ้งสร้างใหม่ (คู่ +/− ในประวัติคือขยะที่ต้องอ่านข้ามทุกครั้ง) · หลังปิดงวด **ห้ามทั้ง delete และ reversal** เพราะกระทบยอดปรับยอดรวมไปแล้ว ลบต้นทางอีก = หักลบซ้ำสองรอบ (บั๊กนับซ้ำที่ `last_reconciled_at` กันมาตั้งแต่แรก) → 409 · ขาโยกลบขาเดียว = ยอดสองกระเป๋าไม่บาลานซ์ จึงลบทั้งคู่ใน batch และเช็คงวดปิดของทั้งสองก่อน · `reverses_id` ย้าย v3 (บอท/สลิปที่ระบบสร้างเองอาจต้องมีร่องรอยแก้) · `deleted_at` = เวลาที่กด (`nowIso`) ไม่ใช่วันที่รายการ |
